...
def main():
    ...
    if TRACKING or TRACK_BASIC:
        ############### TRACKING PART: READING FILES
        ###################
        dataset_dir = os.path.join(DATA_DIRECTORY)
        resolution = '480p'
        # person_all = pickle_load(os.path.join(dataset_dir, 'PersonSearch', resolution, SPLIT + '.pkl'), encoding='latin')
        # object_all = pickle_load(os.path.join(dataset_dir, 'ObjectSearch', resolution, SPLIT + '.pkl'), encoding='latin')
        # category_all = pickle_load(os.path.join(dataset_dir, 'Class', resolution, SPLIT + '.pkl'), encoding='latin')
        frame_cnt = 0

        ##### LOADING MODEL
        global pred_prob, frames, flow1, flow2, orig_mask, \
            model_prop, instance_num, fr_h_r, fr_w_r, appear, \
            location, frame_0, state

        if FOR_BAC:
            cfg = Config.from_file('reidnew/configs/test_config.py')
            model_prop = []
            model_prop = getattr(models, cfg.model.name)(cfg.model)
            if os.path.isfile(os.path.join('reidnew', cfg.weight)):
                print(("=> loading checkpoint '{}'".format(os.path.join('reidnew', cfg.weight))))
                checkpoint = torch.load(os.path.join('reidnew', cfg.weight))
                model_prop.load_state_dict(checkpoint['state_dict'], True)
                print(("=> loaded checkpoint"))
            else:
                print(("=> no checkpoint found at '{}'".format(os.path.join('reidnew', cfg.weight))))
            model_prop = model_prop.to(torch.device('cuda:{}'.format(1)))
            model_prop.eval()
            cudnn.benchmark = True
    ...
    for i_iter, batch in enumerate(testloader):
        ...
        with torch.no_grad():
            ...
            for idx in tqdm(range(0, nFrames), total=nFrames, desc='Evaluating the video:{}'.format(name[0]), ncols=120,
                ...
                #             last_seen[l] = cv2.resize(mask, (gt.shape[1], gt.shape[0]), interpolation=cv2.INTER_NEAREST)
                #         else:
                #             last_seen[l] = mask.copy()
                #     else:
                #         # last_seen[l] = getBB(mask=last_seen[l], margin=60).astype(np.float)
                #         last_seen[l] = np.ones_like(mask)
                # ####################################################################################
